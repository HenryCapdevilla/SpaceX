...
const [launches, setLaunches] = useState([]);
  ...
  const [dateFilterType, setDateFilterType] = useState(["exact", "year"])
  ...
  const filteredCards = launches.filter((l) => {
    ...
    if (cardFilterField === "launch_date" && cardFilterValue) {
      const launchDate = new Date(l.launch_date); // convertir la fecha del backend
    
      if (dateFilterType === "exact") {
        return launchDate.toISOString().slice(0, 10) === cardFilterValue;
      } else if (dateFilterType === "year") {
        return launchDate.getFullYear().toString() === cardFilterValue;
      }
    }
      ...
      <div className="min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-200 transition-colors duration-300 p-6">
        ...
        <div className="mb-6">
          ...
          {filteredSuggestions.slice(0, 10).map((sug, idx) => {
            const displayText =
              dateFilterType === "year"
                ? new Date(sug).getFullYear().toString()
                : sug;
            ...
            return (
              <button
                key={idx}
                onClick={() => {
                  setCardFilterValue(displayText);
                  setCurrentPage(1);
                }}
